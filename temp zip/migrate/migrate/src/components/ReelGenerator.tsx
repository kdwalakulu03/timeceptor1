...
// ─── Main Component ───────────────────────────────────────────────────────────
export function ReelGenerator({ slides }: ReelGeneratorProps) {
  ...
  const stopEarly = useCallback(() => { abortRef.current = true; }, []);
    ...
    <>
      {isWorking && (
        ...
        >
          ...
          <p className="font-mono text-[9px] uppercase tracking-[0.35em] text-[#C1A661]/60">
            {statusText}
          </p>
          ...
          <div className="flex flex-col items-center gap-2 mt-2">
            <div className="w-48 h-1.5 bg-white/10 rounded-full overflow-hidden">
              <div
                className="h-full bg-[#C1A661] rounded-full transition-all duration-200"
                style={{ width: `${progress}%` }}
              />
            </div>
            <span className="font-mono text-[10px] text-white/60 uppercase tracking-widest">
              {status === 'capturing' ? 'Capturing' : 'Encoding'} {progress}%
            </span>
            <button
              onClick={stopEarly}
